...
typedef struct {
    ...
    XSneakysnake_bit_uio_map maps[ MAX_UIO_MAPS ];
} XSneakysnake_bit_uio_info;

/***************** Variable Definitions **************************************/
static XSneakysnake_bit_uio_info uio_info;
...
static int uio_info_read_name(XSneakysnake_bit_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSneakysnake_bit_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSneakysnake_bit_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSneakysnake_bit_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSneakysnake_bit_Initialize(XSneakysnake_bit *InstancePtr, const char* InstanceName) {
	XSneakysnake_bit_uio_info *InfoPtr = &uio_info;
